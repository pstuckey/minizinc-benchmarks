...
Each clue for a row or column is a sequence of 0s and 1s followed by filler -1s to make each clue the same length. These sequence of filled cells in that row or column, in order from left to right (for rows) or top to bottom (for columns). For example, a clue of `[1, 1, 1, 0, 1]` means there is a run of exactly 3 filled cells followed (after at least one empty cell) by a run of exactly 1 filled cell. There must be at least one empty cell between any two consecutive runs. Note this is not the standard representation of nonogram clues which would be [3, 1]. A version of the DFA construction using the more standard representation is given in the model but not used.
...
| `rows`    | A 2D array of size `Y × maxlen` giving the run-length clues for each row. A value of `-1` indicates a padding entry with no meaning. |
...
| -------- | ------------------------------- | ----------------------------------------------------------------------- |
| `A`      | `array[1..Y, 1..X] of var CELL` | The grid itself. Each cell takes the value `1` (empty) or `2` (filled). |
...
Each individual row or column constraint is enforced using the  MiniZinc global constraint `regular`. The `regular` constraint checks that a sequence of variables corresponds to a word accepted by a finite automaton (a DFA). The model constructs the DFA transition table on-the-fly from the clue sequence that encode how DFA states should transition depending on consecutive clue values.
...
- ` ` represents an empty cell (value `B`)
- `#` represents a filled cell (value `F`)
